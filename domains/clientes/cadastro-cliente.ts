...
function adicionarTelefone(valor: string = ''): void {
  const container = document.getElementById('telefones-cliente') as HTMLElement
...
function removerEsteTelefone(btn: HTMLButtonElement): void {
...
function salvarCliente(): void {
  ...
  try {
    ...
    showAviso((e as Error).message || "Dados inválidos!", "#ef4444")
    return;
  }
...
function atualizarListaClientes(): void {
  const ul = document.getElementById('lista-clientes') as HTMLElement
  const busca = (document.getElementById('buscaCliente') as HTMLInputElement).value.trim().toLowerCase()
...
function abrirClienteDetalhe(idx: number): void {
  ...
  `;
  (document.getElementById("form-cadastro-cliente") as HTMLElement).style.display = "none"
}
function fecharClienteDetalhe(): void {
  (document.getElementById("cliente-detalhe") as HTMLElement).style.display = "none"
  (document.getElementById("form-cadastro-cliente") as HTMLElement).style.display = "block"
}

function excluirCliente(idx: number): void {
